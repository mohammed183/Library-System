	...
	public AddStudent(Data data) {
		...
		passwordtext.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				if(passwordtext.getText().length() >= 15) { // if length is 15 or above
					passwordMessage.setForeground(Color.GREEN);
					passwordMessage.setText("password is Strong");				
				}
				else if(passwordtext.getText().length() >= 8) { // if length is 8 or above
					passwordMessage.setForeground(Color.ORANGE);
					passwordMessage.setText("password is Good");
				}
				else if(passwordtext.getText().length() < 8) { // if length is less than 8
					passwordMessage.setForeground(Color.RED);
					passwordMessage.setText("minimum 8 letters or digits!!");
				}
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (nametext.getText().equals("") || passwordtext.getText().equals("") || emailText.getText().equals("")
					...
					{
						JOptionPane.showMessageDialog(null, "password is too weak!", "Message",
								JOptionPane.PLAIN_MESSAGE);
					}
					else if (valid) {
		...
		passwordMessage1 = new JLabel("");
		passwordMessage1.setForeground(Color.RED);
		passwordMessage1.setFont(new Font("Tahoma", Font.PLAIN, 8));
